...
const pipe2 = (first, ...rest) =>
  pipe([].concat(first, rest).map(f => (...args) => Promise.resolve(f(...args))))()
...
const renderTemplateIfValid = (fs, root, template, templateArgs) => {
  ...
  return fs
    .ensureDir(baseDir)
    .then(() => fs.outputFile(filename, template.content(templateArgs)))
}
...
const generateWithOptions = ({
  ...
  if (generateExample) {
    ...
    const errorRemedyMessage =
      'both react-native-cli and yarn CLI tools are needed to generate example project'
        ...
        pipeWithArgs(
          ...
          templateArgs =>
            Promise.all(
              templates
                .filter(template =>
                  template.platform ? platforms.includes(template.platform) : true
                )
  ...
  const generateExampleWithName = () =>
    ...
    pipeWithArgs(
      {
        // exec command args:
        exampleReactNativeInitCommand: `react-native init ${exampleName} --version ${exampleReactNativeVersion}`,
...
module.exports = options => generateWithOptions(normalizedOptions(options))
